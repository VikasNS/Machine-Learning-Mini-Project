...
model=Sequential()
model.add(Embedding(input_dim=733,output_dim=40,input_length=22))
...
Dropout(0.4)
model.add(Bidirectional(LSTM(500,return_sequences=True,activation='relu'),merge_mode='concat'))
Dropout(0.4)
...
with open('trigram_test','rb') as X_test:
...
#model.fit_generator(generator=cus_gen(32,X,y),steps_per_epoch=(100000),epochs=100,callbacks=[call])
