...
def clean_tokenize(data_frame,min_len=5,max_len=10):
    ...
    for c,lines in enumerate(data_frame[:].lyrics):
        if(isinstance(lines,str)):
            ...
            for line in lines:
                ...
                if (max_len > len(words) > min_len):
                    data.append(words)
                    words_in_data += words
            print('Building sentences', c / 3800, "%")
...
data,words_in_data,words_in_embedding,missing_words_in_data=clean_tokenize(read_file('lyrics.csv'))
...
Test=['evolution','specification','implementation','validation','operation']
from itertools import permutations


Test=list(permutations(Test))
